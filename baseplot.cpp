...
{
    if (MS.length() < RMSLength && SensorValue.length() != 0)
    {
        MS.append((SensorValue.last() * SensorValue.last()) / RMSLength);
        MSSum += MS.last();
    }
    if (MS.length() >= RMSLength)
    {
        if (RMSCounter == RMSLength)
            RMSCounter = 0;
...
{
    if (MS1.length() < RMSLength && SensorValue1.length() != 0)
    {
    ...
    if (MS2.length() < RMSLength && SensorValue2.length() != 0)
    {
...
void ThreeGraph::ParseData(QVector<double> &Sensor1Value, QVector<double> &Sensor2Value, QVector<double> &Sensor3Value, int32_t temp1, int32_t temp2, int32_t temp3)
{
    ...
    Sensor1Value.append(((double)temp1) / ConversionFactor); // dividing by RMSLength000 to convert from Pa to bar
    Sensor2Value.append(((double)temp2) / ConversionFactor); // dividing by RMSLength000 to convert from Pa to bar
    Sensor3Value.append(((double)temp3) / ConversionFactor); // dividing by RMSLength000 to convert from Pa to bar
    DataCounter++;
}
...
void ThreeGraph::PlotData(QVector<double> &XAxisData, QVector<double> &YAxisData1, QVector<double> &YAxisData2, QVector<double> &YAxisData3)
{
    ...
    else
    {
        ...
        PreviousTimeCounter++;
        Population++;
        if (Population >= WindowSize) // check if we have RMSLength0 data points in our graph if we do start clearing the oldest value
            Populated = true;
    }
}
...
void ThreeGraph::CalculateRMS(QVector<double> &SensorValue1, QVector<double> &SensorValue2, QVector<double> &SensorValue3)
{
    ...
    if (MS3.length() < RMSLength && SensorValue3.length() != 0)
    {
...
/*
The following methods are called automatically by plot data.It calculates 
Global and Local Maxima/Minma and RMS
*/
void MasterPlot::UpdateMinMax(QVector<double> &YAxisData1, QVector<double> &YAxisData2, QVector<double> &YAxisData3)
{
    ...
    // the following changes will only be reflected when the graph is updated by calling the
    // replot and update functions
    Plot->graph(0)->setName(Graph1Name + " GMax\t" + QString::number(GMaxVal1, 'f', 2) + "\t" + "GMin\t" + QString::number(GMinVal1, 'f', 2) + "\t" + " LMax\t" + QString::number(LMaxVal1, 'f', 2) + "\t" + "LMin\t" + QString::number(LMinVal1, 'f', 2) + "\t" + " RMS\t" + QString::number(sqrt(MS1Sum), 'f', 2));

    Plot->graph(1)->setName(Graph2Name + " GMax\t" + QString::number(GMaxVal2, 'f', 2) + "\t" + "GMin\t" + QString::number(GMinVal2, 'f', 2) + "\t" + " LMax\t" + QString::number(LMaxVal2, 'f', 2) + "\t" + "LMin\t" + QString::number(LMinVal2, 'f', 2) + "\t" + " RMS\t" + QString::number(sqrt(MS2Sum), 'f', 2));

    Plot->graph(2)->setName(Graph3Name + " GMax\t" + QString::number(GMaxVal3, 'f', 2) + "\t" + "GMin\t" + QString::number(GMinVal3, 'f', 2) + "\t" + " LMax\t" + QString::number(LMaxVal3, 'f', 2) + "\t" + "LMin\t" + QString::number(LMinVal3, 'f', 2) + "\t" + " RMS\t" + QString::number(sqrt(MS3Sum), 'f', 2));
}
...
void MasterPlot::UpdateMinMax(QVector<double> &YAxisData1, QVector<double> &YAxisData2)
{
    ...
    Plot->graph(0)->setName(Graph1Name + " GMax " + QString::number(GMaxVal1) + " GMin " + QString::number(GMinVal1) + " LMax " + QString::number(LMaxVal1) + " LMin " + QString::number(LMinVal1) + " RMS " + QString::number(sqrt(MS1Sum)));

    Plot->graph(1)->setName(Graph2Name + " GMax " + QString::number(GMaxVal2) + " GMin " + QString::number(GMinVal2) + " LMax " + QString::number(LMaxVal2) + " LMin " + QString::number(LMinVal2) + " RMS " + QString::number(sqrt(MS2Sum)));
}
